...
##################################################################################################################
#
findmin <- function(basis,model=NULL,coef=NULL,vcov=NULL,at=NULL,from=NULL,to=NULL,by=NULL,sim=FALSE,nsim=1000) {
  ...
  if(!any(class(basis)%in%c("crossbasis","pbs")))
    stop("the first argument must be an object of class 'crossbasis' or 'pbs'")
