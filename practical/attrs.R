...
################################################################################
attrs <- function(x,basis,cases, data,model=NULL,coef=NULL,vcov=NULL,type,
 tot=TRUE,sim=FALSE,nsim=1000) {
################################################################################
  type <- match.arg(type,c("an","af"))

  # cases:
  cases<-as.data.frame(cbind(x,cases))
  cases<-matrix(by(cases$cases,cases$x,FUN = mean))
################################################################################
#
...
#
  ...
  if(!any(class(basis)%in%c("pbs")))
    stop("the first argument must be an object of class 'pbs'")
  ...
  if(sim) {
    ...
    afsim <- apply(coefsim,2, function(coefi) {
      ...
      if(tot) {
        isna <- is.na(ani)
        ani <- sum(ani,na.rm=T)
        afi <- ani/sum(cases[!isna],na.rm = T)
      }
      sum(ani[!is.na(ani)],na.rm = T)/sum(cases[!is.na(ani)],na.rm = T)
    })
    #ansim <- afsim*den
    ansim <- afsim
  }
